...
function process(
    ...
    if (typeName) {
        ...
        const normalSchema = normalizeSchema(schema);
        const validator = printTypeCollectionValidator(
            symbols,
            useNamedExport,
            normalSchema,
            resolve(sourceFile),
...
export default function run(args?: string[]) {
    ...
    const fileMapedToOut = files.reduce((acc, {fileName, typeName}) => {
        ...
        acc[data.outputFileName] = uniqBy(
            [...acc[data.outputFileName], data],
            'typeName',
        );
