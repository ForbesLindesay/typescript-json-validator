...
describe('Print Validator tests', () => {
	beforeAll(async () => {
		...
		const { symbols, schema, symbolsByFile } = parse([__dirname + '/../ComplexExample.ts'], tsConfig).getAllTypes();

		const output = __dirname + '/output/ComplexExample.validator.ts';
		writeFileSync(
			output,
			printTypeCollectionValidator(symbols, false, schema, '../../ComplexExample', symbolsByFile, tsConfig, 'fast', {
				ajv: {},
  ...
  `,
	...
	test('validateValue', () => {
		...
		expect(() => validate.validate('MyEnum')(42)).toThrowErrorMatchingInlineSnapshot(`"Invalid MyEnum: MyEnum must be equal to one of the allowed values"`);
	});
	...
	// 				},
	// 			} as any),
	// 		).toThrowErrorMatchingInlineSnapshot(`"validate.validateKoaRequest is not a function"`);
